    ...
    return {
        ...
        envelope: function(options) {
            ...
            */
            ...
            console.error("gcode:", gcode.userData.lines);

            // JC: WARNING! Assuming gcode is in mm because we always use mm for grooving!
            console.warn("ASSUMING G21");
            var newgcode = ["G21"];
            ...
            console.warn("ctr:", ctr, "len of newgcode arr (should be same as ctr):", newgcode.length);
        ...
        onDisplay: function() {
            //this.autoFillData(); // JC: don't autofill because I set the defaults myself in init()
            this.formUpdate();
        },
        ...
        doNextStep: function() {
            ...
            if (this.currentStep >= this.probes.length) {
                ...
                this.send("G1 f5000 Z" + $("#com-chilipeppr-widget-myautolevel .high-z").val() + "\n"); //raise probe
                this.send("G1 f5000 X" + 0 + " Y" + 0 + "\n"); //move home
              
                // JC: Convert probe data
                var that = this;
                var websocket = new WebSocket("ws://127.0.0.1:6789/");
                websocket.onopen = function() {
                    console.error(JSON.stringify(that.probes, null, " "));
                    websocket.send(JSON.stringify(that.probes, null, " "));
                };
